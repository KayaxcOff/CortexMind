...
void ScalarKernel::add(const f32* Xx, const f32 value, f32* Xz, const size_t N) {
    ...
    f32x4* Xz = reinterpret_cast<f32x4*>(Xz);

    kernels::scalar<ops::Addition><<<grid1d(N), BLOCK_SIZE_1D>>>(Xx4, value, Xz, N);
}
...
void ScalarKernel::sub(const f32* Xx, const f32 value, f32* Xz, const size_t N) {
    ...
    kernels::scalar<ops::Subtraction><<<grid1d(N), BLOCK_SIZE_1D>>>(Xx4, value, Xz, N);
}
...
void ScalarKernel::mul(const f32* Xx, const f32 value, f32* Xz, const size_t N) {
    ...
    kernels::scalar<ops::Multiplication><<<grid1d(N), BLOCK_SIZE_1D>>>(Xx4, value, Xz, N);
}
...
void ScalarKernel::div(const f32* Xx, const f32 value, f32* Xz, const size_t N) {
    ...
    kernels::scalar<ops::Division><<<grid1d(N), BLOCK_SIZE_1D>>>(Xx4, value, Xz, N);
}