...
TrackedMem::TrackedMem(const size_t capacity) : m_capacity(capacity), m_used(0) {
    this->m_buffer = static_cast<f32*>(operator new(capacity * sizeof(f32), static_cast<std::align_val_t>(32), std::nothrow));
    CXM_ASSERT(this->m_buffer == nullptr, "Memory allocation failed");
