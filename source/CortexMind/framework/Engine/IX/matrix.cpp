...
void MatrixOp::dispatch(
    ...
    const BroadcastKind kind, const DeviceType dev, const char op)
{
    const size_t N  = compute_size(shape_z);
    const size_t M  = shape_z.size() >= 2
        ? static_cast<size_t>(shape_z[shape_z.size() - 2]) : 1;
    const auto Nc = static_cast<size_t>(shape_z.back());

    #define CXM_DISPATCH_BACKEND(avx_fn, cuda_fn)               \
        do {                                                      \
            if (dev == DeviceType::kHOST) { avx_fn; }           \
            else {                                                \
    ...
    switch (kind) {
        case BroadcastKind::kNone:
            if (op == '+') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::matrix_t::add(x, y, z, N);
                    else cuda::Matrix::add(x, y, z, N);
                #else
                    avx2::matrix_t::add(x, y, z, N);
                #endif
            } else if (op == '-') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::matrix_t::sub(x, y, z, N);
                    else cuda::Matrix::sub(x, y, z, N);
                #else
                    avx2::matrix_t::sub(x, y, z, N);
                #endif
            } else if (op == '*') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::matrix_t::mul(x, y, z, N);
                    else cuda::Matrix::mul(x, y, z, N);
                #else
                    avx2::matrix_t::mul(x, y, z, N);
                #endif
            } else {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::matrix_t::div(x, y, z, N);
                    else cuda::Matrix::div(x, y, z, N);
                #else
        ...
        case BroadcastKind::kRow:
            if (op == '+') {
                    ...
                    else cuda::Broadcast::row_add(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::row_add(x, y, z, M, Nc);
                #endif
            } else if (op == '-') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::row_sub(x, y, z, M, Nc);
                    else cuda::Broadcast::row_sub(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::row_sub(x, y, z, M, Nc);
                #endif
            } else if (op == '*') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::row_mul(x, y, z, M, Nc);
                    else cuda::Broadcast::row_mul(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::row_mul(x, y, z, M, Nc);
                #endif
            } else {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::row_div(x, y, z, M, Nc);
                    else cuda::Broadcast::row_div(x, y, z, M, Nc);
                #else
        ...
        case BroadcastKind::kCol:
            if (op == '+') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::col_add(x, y, z, M, Nc);
                    else cuda::Broadcast::col_add(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::col_add(x, y, z, M, Nc);
                #endif
            } else if (op == '-') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::col_sub(x, y, z, M, Nc);
                    else cuda::Broadcast::col_sub(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::col_sub(x, y, z, M, Nc);
                #endif
            } else if (op == '*') {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::col_mul(x, y, z, M, Nc);
                    else cuda::Broadcast::col_mul(x, y, z, M, Nc);
                #else
                    avx2::Broadcast::col_mul(x, y, z, M, Nc);
                #endif
            } else {
                #if CXM_IS_CUDA_AVAILABLE
                    if (dev == DeviceType::kHOST) avx2::Broadcast::col_div(x, y, z, M, Nc);
                    else cuda::Broadcast::col_div(x, y, z, M, Nc);
                #else
...
void MatrixOp::add(
    const TensorStorage* Xx, const std::vector<i64>& shape_x, const std::vector<i64>& stride_x,
    ...
    TensorStorage*       Xz, const std::vector<i64>& shape_z, const std::vector<i64>& stride_z)
{
    ...
    CXM_ASSERT(Xx->device() != Xz->device(),
        "Device mismatch: Xx=" + as_string(Xx->device()) + " Xz=" + as_string(Xz->device()));

    dispatch(Xx->data(), Xy->data(), Xz->data(),
             shape_x, stride_x, shape_y, stride_y, shape_z, stride_z,
             classify_broadcast(shape_x, shape_y), Xx->device(), '+');
}
...
void MatrixOp::add(
    TensorStorage*       Xx, const std::vector<i64>& shape_x, const std::vector<i64>& stride_x,
    const TensorStorage* Xy, const std::vector<i64>& shape_y, const std::vector<i64>& stride_y)
{
...
void MatrixOp::matmul(
    const TensorStorage* Xx, const TensorStorage* Xy,
    TensorStorage* Xz,
    ...
    const DeviceType dev)
{
    ...
    CXM_ASSERT(Xx->device() != Xy->device(),
        "Device mismatch: Xx=" + as_string(Xx->device()) + " Xy=" + as_string(Xy->device()));
