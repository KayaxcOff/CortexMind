...
namespace cortex::_fw::cuda::kernels {
    ...
    __global__ void row_broadcast(const f32* __restrict__ Xx, const f32* __restrict__ Xy, f32* __restrict__ Xz, const size_t M, const size_t N, const OpType op) {
     ...
     */
    template<typename OpType>
    __global__ void row_broadcast_ip(f32* __restrict__ Xx, const f32* __restrict__ Xy, const size_t M, const size_t N, const OpType op) {
        const size_t col = blockIdx.x * blockDim.x + threadIdx.x;
        const size_t row = blockIdx.y * blockDim.y + threadIdx.y;
     ...
     */
    template<typename OpType>
    __global__ void col_broadcast(const f32* __restrict__ Xx, const f32* __restrict__ Xy, f32* __restrict__ Xz, const size_t M, const size_t N, const OpType op) {
