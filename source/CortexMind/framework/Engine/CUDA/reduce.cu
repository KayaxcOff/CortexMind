...
void ReduceOp::sum(const f32* Xx, f32* Xz, const size_t N) {
...
void ReduceOp::mean(const f32* Xx, f32* Xz, const size_t N) {
    sum(Xx, Xz, N);

    ScalarKernel::div(Xz, static_cast<f32>(N), 1);
}

void ReduceOp::var(const f32* Xx, f32* Xz, const size_t N) {
    f32* d_mean;
    f32* d_workspace;

    malloc(reinterpret_cast<void**>(&d_mean), sizeof(f32));
    malloc(reinterpret_cast<void**>(&d_workspace), N * sizeof(f32));
    ...
    sum(d_workspace, Xz, N);

    ScalarKernel::div(Xz, static_cast<f32>(N), 1);

    free(reinterpret_cast<void**>(&d_mean));
    free(reinterpret_cast<void**>(&d_workspace));
}

void ReduceOp::stdv(const f32* Xx, f32* Xz, const size_t N) {
    var(Xx, Xz, N);
    ElementWise::sqrt(Xz, Xz, 1);
}

void ReduceOp::argmax(const f32* Xx, i32* Xz, const size_t N) {
...
void ReduceOp::argmin(const f32* Xx, i32* Xz, const size_t N) {
    constexpr size_t shared_mem = (BLOCK_SIZE_1D / WARP_SIZE) * sizeof(kernels::KeyValuePair);
    kernels::ArgMin<<<1, BLOCK_SIZE_1D, shared_mem>>>(Xx, Xz, N);
}


void ReduceOp::sum_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceSumDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::mean_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceMeanDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::var_dim(const f32* Xx, f32* Xz, const f32* means, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceVarDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, means, outer, dim, inner);
}

void ReduceOp::stdv_dim(const f32* Xx, f32* Xz, const f32* means, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceStdvDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, means, outer, dim, inner);
}

void ReduceOp::min_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceMinDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::max_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceMaxDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::argmax_dim(const f32* Xx, i32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceArgMaxDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::argmin_dim(const f32* Xx, i32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceArgMinDim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::norm1_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
    kernels::ReduceNorm1Dim<BLOCK_SIZE_1D><<<grid_size, BLOCK_SIZE_1D>>>(Xx, Xz, outer, dim, inner);
}

void ReduceOp::norm2_dim(const f32* Xx, f32* Xz, size_t outer, size_t dim, size_t inner) {
    size_t grid_size = outer * inner;
