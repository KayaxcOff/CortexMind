...
MindTensor::MindTensor() : m_device(deviceType::host), m_grad_flag(false) {
    this->storage_ = nullptr;
    this->flow_ = nullptr;
    this->gradient_ = nullptr;
}

MindTensor::MindTensor(const std::vector<i64> &shape, const deviceType device, const bool requires_grad) : m_device(device), m_grad_flag(requires_grad) {
...
MindTensor::MindTensor(const std::shared_ptr<TensorStorage> &tensor_storage, const bool requires_grad) : m_device(tensor_storage->device()), m_grad_flag(requires_grad) {
...
MindTensor::MindTensor(const std::vector<i64> &shape, const f32 *data, const deviceType device, const bool requires_grad) : m_device(device), m_grad_flag(requires_grad) {
    this->storage_ = std::make_shared<TensorStorage>(compute_numel(shape), device);
...
MindTensor::MindTensor(const MindTensor &other) : m_device(other.m_device), m_grad_flag(other.m_grad_flag) {
...
MindTensor::MindTensor(MindTensor &&other) noexcept : m_device(other.m_device), m_grad_flag(other.m_grad_flag) {
